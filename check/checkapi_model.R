...
countryList= "Malawi"#unique(surveys$country)
indicatorlist=infolist$ID
...
for (i in which(surveys$country %in% countryList)) { #seq_len(nrow(surveys))
    ...
    if (file.exists(qfile)) {
      ...
      loaded_data1 <- qread(qfile)
        ...
        all_results[[length(all_results) + 1]] <- data.frame(
          ...
          year          = year,
          indicator=indicator,
          surveyPrev=    as.numeric(loaded_data1$res.natl$direct.est),
  ...
  write.csv(summary_df, file.path(source_path, "Gates-results/check", country,year,paste0(country,"Checkapi1.csv")), row.names = FALSE)
...
csv_files <- list.files(
  check_root,
  pattern = "Checkapi1\\.csv$",
  recursive = TRUE,
  full.names = TRUE
)

# 2. read and combine
all_checkapi1 <- map_dfr(csv_files, function(f) {
...
#1. no data 
all_checkapi1_nodata <- all_checkapi1 %>%
...
all_checkapi1_eyeballcheck <- all_checkapi1 %>%
  semi_join(keys, by = c("country", "year", "indicator"))



all_checkapi1_HC_WIXQ_P_12Q=all_checkapi1[ all_checkapi1$indicator=="HC_WIXQ_P_12Q",]
#all_checkapi0_HC_WIXQ_P_12Q$surveyPrev



all_checkapi1_issue=all_checkapi1[(is.na(all_checkapi1$match) | all_checkapi1$match == 0)& all_checkapi1$indicator!="HC_WIXQ_P_12Q",]


all_checkapi1_issue_smallset <- all_checkapi1_issue %>%
  anti_join(all_checkapi1_eyeballcheck, by = c("country", "year", "indicator")) %>%
  anti_join(all_checkapi1_nodata,        by = c("country", "year", "indicator"))
...
all_checkapi1_new <- all_checkapi1_issue_smallset %>%
...
write.csv(
  all_checkapi1,
  file.path(check_root, "Checkapi_ALL_after.csv"),
  row.names = FALSE
)


# write.csv(
#   all_checkapi1_nodata,
#   file.path(check_root, "Checkapi_NODATA_after.csv"),
#   row.names = FALSE
# )
# 
# 
# write.csv(
#   all_checkapi1_eyeballcheck,
#   file.path(check_root, "Checkapi_eyeballchecked_after.csv"),
#   row.names = FALSE
# )

write.csv(
  all_checkapi1_issue_smallset,
  file.path(check_root, " s"),
  row.names = FALSE
)
