...
#'
#' @export
directEST_1030 = function (data, cluster.info, admin, strata = "all", CI = 0.95,
                           ...
                           ...)
{
  ...
  compute_domain_summaries <- function(agg_cluster_in_out, n_h){
    ...
    df <- v_h_c_dot %>%
      left_join(
        v_h_dot_dot %>%
          dplyr::select(admin1.name, strata, v_h_dot_dot),
        by = c("admin1.name", "strata")
      )%>%
      left_join(
        v_dot_dot_dot %>%
          dplyr::select(admin1.name, v_dot_dot_dot),
        by = c("admin1.name")
      )%>%
      left_join(
        n_h %>%
          dplyr::select(admin1.name, strata, n_h),
        by = c("admin1.name", "strata")
      )%>%
      left_join(
        p_h_c_hj_hat %>%
      ...
      left_join(
        p_h_hj_hat %>%
          dplyr::select(admin1.name, strata, p_h_hj_hat),
        by = c("admin1.name", "strata")
      ) %>%
      left_join(
        p_hj_hat %>%
          dplyr::select(admin1.name, p_hj_hat),
        by = c("admin1.name")
      )
  ...
  compute_domain_summaries_fix <- function(
    ...
    df <- v_h_c_dot %>%
      left_join(
        v_h_dot_dot_aug %>%
          dplyr::select(admin1.name, strata, v_h_dot_dot_aug),
        by = c("admin1.name", "strata")
      )%>%
      left_join(
        v_dot_dot_dot_aug %>%
          dplyr::select(admin1.name, v_dot_dot_dot_aug),
        by = c("admin1.name")
      )%>%
      left_join(
        n_h %>%
      ...
      left_join(
        df_pre_phantom %>%
          dplyr::select(admin1.name, strata, phantom_to_add),
        by = c("admin1.name", "strata")
      )%>%
      left_join(
        p_h_c_hj_hat %>%
      ...
      left_join(
        p_h_aug_hat %>%
          dplyr::select(admin1.name, strata, p_h_aug_hat),
        by = c("admin1.name", "strata")
      ) %>%
      left_join(
        p_aug_hat %>%
          dplyr::select(admin1.name, p_aug_hat),
        by = c("admin1.name")
      )
