...
# Paths (run this script from within SAE4LMIC so here() resolves correctly)
git_path    <- here::here()          # SAE4LMIC repo
source_path <- dirname(git_path)     # GATES root
...
output_dir  <- file.path(source_path, "Gates-results", "Reports")
countrylist1=c("Malawi")

for (cty in countrylist1) {
  message("Rendering report for: ", cty)
  render(
    input  = file.path(git_path, "report", "report.Rmd"),
...
for (cty in countrylist1) {
  message("Rendering report for: ", cty)
  render(
    input  = file.path(git_path, "report", "report_short.Rmd"),
    output_format = rmarkdown::pdf_document(toc = TRUE, number_sections = FALSE, keep_tex = TRUE),
    params = list(
      country     = cty,
      source_path = source_path,
...
for (cty in countrylist1) {
  ...
  render(
    input  = file.path(git_path, "report", "report_anytwoyear.Rmd"),
    output_format = rmarkdown::pdf_document(toc = TRUE, number_sections = FALSE, keep_tex = TRUE),
    params = list(
      country     = cty,
      source_path = source_path,
...
countrylist1 = c("Ethiopia")
sel_yr1 <- 2019   # old year (NULL = min for the country)
sel_yr2 <- 2024   # new year (NULL = max for the country)
output_dir  <- file.path(source_path, "Gates-results", "Reports")
